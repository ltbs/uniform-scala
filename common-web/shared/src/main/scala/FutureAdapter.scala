  ...
  def alwaysRerun = new ~>[Future, WM] {
    def apply[A](fa: Future[A]): WM[A] = new WM[A] {
      def apply(pageIn: PageIn)(implicit ec: ExecutionContext): Future[PageOut[A,Html]] =
        fa.map{ x =>
          import pageIn._
          PageOut(path, state, AskResult.Success(x), pageIn.pathPrefix)
        }
    }
  }
    ...
    def apply[A](fa: Future[A])(implicit codec: Codec[A]): WM[A] = new WM[A] {
      ...
      def apply(pageIn: PageIn)(implicit ec: ExecutionContext): Future[PageOut[A,Html]] = {
        ...
        if (oldTrigger == Some(trigger)) {
          ...
          PageOut(path, state, AskResult.Success[A, Html](oldie), pageIn.pathPrefix).pure[Future]
        } else {
          fa.map{ result =>
            ...
            PageOut(path, state ++ newData, AskResult.Success[A, Html](result), pageIn.pathPrefix)
