...
object Rule extends Quantifiable.ToQuantifiableOps {
  ...
  case class MinLength[A: Quantifiable] private[validation] (len: Int, errorMsg: String) extends Rule[A] {
  ...
  case class MaxLength[A: Quantifiable] private[validation] (len: Int, errorMsg: String) extends Rule[A] {
    def apply(in: A): Validated[ErrorTree, A] =
      Validated.cond(in.qty <= len, in, error(errorMsg, in.qty, len))
  }
  ...
  case class AlwaysPass[A] private[validation] () extends Rule[A] {
    def apply(in: A): Validated[ErrorTree, A] =
      Validated.Valid(in)
  }
  ...
  case class AlwaysFail[A] private[validation] () extends Rule[A] {
    def apply(in: A): Validated[ErrorTree, A] =
  ...
  case class LengthBetween[A: Quantifiable] private[validation] (min: Int, max: Int) extends Rule[A] {
    def apply(in: A): Validated[ErrorTree, A] =
  ...
  case class NonEmpty[A: Empty] private[validation] (errorMsg: String) extends Rule[A] {
    def apply(in: A): Validated[ErrorTree, A] =
  ...
  case class MatchesRegex private[validation] (regex: String, errorMsg: String) extends Rule[String] {
  ...
  case class Min[A: Order] private[validation] (minValue: A, errorMsg: String) extends Rule[A]{
    def apply(in: A): Validated[ErrorTree, A] =
  ...
  case class Max[A: Order] private[validation] (maxValue: A, errorMsg: String) extends Rule[A]{
    def apply(in: A): Validated[ErrorTree, A] =
  ...
  case class Between[A: Order] private[validation] (minValue: A, maxValue: A) extends Rule[A] {
    def apply(in: A): Validated[ErrorTree, A] =
